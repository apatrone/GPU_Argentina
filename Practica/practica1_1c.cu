...
float h_c=2; //host 
__constant__ float d_c; //device

     
__global__ void multiply( float array[] , const int thread_number)
{
...
int main(int argc, char *argv[]){
    ...
    // pointers to host & device arrays
	  ...
	  // malloc a host array
      host_array = (float*)malloc( h_array_size * sizeof(float));

    for(int i=0; i<h_array_size; i++){
        host_array[i]= static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/10)); //random float between 0 and 10
        printf("%f\t", host_array[i]);
    }
    printf("\n");

    // cudaMalloc a device array
    cudaMalloc(&device_array,h_array_size * sizeof(float));    
    // download and inspect the result on the host:
    cudaMemcpy(device_array, host_array, sizeof(float)*h_array_size, cudaMemcpyHostToDevice);         
    ...
    // download and inspect the result on the host:
    cudaMemcpy(host_array, device_array, sizeof(float)*h_array_size, cudaMemcpyDeviceToHost); 
   ...
   // deallocate memory
	...
	printf("Time elapsed: %f seconds\n", (((float)clock() - (float)time_begin) / 1000000.0F ) * 1000  );//1.141s
